 ...
 *
 * й Copyright 2000 Beatnik, Inc, All Rights Reserved.
			...
			if (totalInstruments)
			{
				...
				if (theErr == noErr)
				{
					...
					if (theI)
					{
						...
						FormatText(text, "\rееее Max LFO's %ld\r", (long)maxLFOs);
						FSWriteCString(fileRef, text);

						FormatText(text, "ееее Max Envelope points %ld\r", (long)maxADSRPoints);
						FSWriteCString(fileRef, text);
						
						FormatText(text, "ееее Max curve points %ld\r", (long)maxCurves);
						FSWriteCString(fileRef, text);
					}
