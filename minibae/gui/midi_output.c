...
bool midi_output_init(const char *client_name, int api_index, int port_index) {
    if(g_rtmidi_out) return true;
    if(!g_midi_out_mutex){ g_midi_out_mutex = SDL_CreateMutex(); }
    ...
    if(!g_rtmidi_out) g_rtmidi_out = rtmidi_out_create_default();
    if(!g_rtmidi_out) return false;
...
void midi_output_shutdown(void){
    if(!g_rtmidi_out) return;
    if(g_midi_out_mutex) SDL_LockMutex(g_midi_out_mutex);
    rtmidi_close_port(g_rtmidi_out);
    rtmidi_out_free(g_rtmidi_out);
    g_rtmidi_out = NULL;
    if(g_midi_out_mutex) SDL_UnlockMutex(g_midi_out_mutex);
}
...
void midi_output_send_all_notes_off(void){
    ...
    for(int ch=0; ch<16; ++ch){
        ...
        msg[0] = (unsigned char)(0xB0 | (ch & 0x0F)); msg[1] = 120; msg[2] = 0;
        midi_output_send(msg, 3);
        // All Notes Off: controller 123
        msg[0] = (unsigned char)(0xB0 | (ch & 0x0F)); msg[1] = 123; msg[2] = 0;
        midi_output_send(msg, 3);
        // Reset controllers (optional): controller 121
        msg[0] = (unsigned char)(0xB0 | (ch & 0x0F)); msg[1] = 121; msg[2] = 0;
        midi_output_send(msg, 3);
    }
}
