// SDL2 GUI for miniBAE – inspired by the BXPlayer GUI.
...
static bool ui_dropdown_two_column(SDL_Renderer *R, Rect r, int *value, const char **items, int count, bool *open,
                                   int mx, int my, bool mdown, bool mclick)
{
    bool changed = false;
    if (count <= 0)
...
static bool g_midi_recording = false;               // are we currently recording incoming MIDI?
static char g_midi_record_path[1024] = {0};         // final .mid path user requested
static char g_midi_record_temp[1024] = {0};         // temporary track data file path
static FILE *g_midi_record_temp_fp = NULL;          // temp file for writing raw track events
static double g_midi_record_start_ts = 0.0;         // timestamp of first recorded event (seconds)
static double g_midi_record_last_ts = 0.0;          // timestamp of last written event (seconds)
static uint64_t g_midi_record_last_pc = 0;          // last performance counter for delta timing
...
// If true, service export at realtime pace instead of fast-batch mode
static bool g_export_realtime_mode = false;
// When true the export is using the lightweight live song (driven by MIDI in) instead of a loaded file
static bool g_export_using_live_song = false;

// PCM-based WAV recording state (used for MIDI-in -> WAV capture)
static bool g_pcm_wav_recording = false;
static bool g_pcm_mp3_recording = false; // platform MP3 recorder active (MIDI-in)
static FILE *g_pcm_wav_fp = NULL;
static uint64_t g_pcm_wav_data_bytes = 0;
static int g_pcm_wav_channels = 2;
static int g_pcm_wav_sample_rate = 44100;
static int g_pcm_wav_bits = 16;
// Track whether we disengaged the audio hardware when starting PCM recording
static bool g_pcm_wav_disengaged_audio = false;
...
static Settings load_settings()
{
    ...
    settings.has_reverb = false;
    settings.reverb_type = 0;
    settings.has_loop = false;
    settings.loop_enabled = true;
    settings.has_volume_curve = false;
    settings.volume_curve = 0;
    settings.has_stereo = false;
    settings.stereo_output = true;
    settings.has_sample_rate = false;
    settings.sample_rate_hz = 44100;
    settings.has_show_keyboard = false;
    settings.show_keyboard = false;
    settings.has_webtv = false;
    settings.disable_webtv_progress_bar = false;
...
{
    if (!g_bae.mixer || !g_bae.bank_loaded || !g_bae.bank_token)
        return NULL;
...
// Simple MIDI file writing helpers
// Create a temporary track data file and open it for appending raw delta+message bytes.
static bool midi_record_start(const char *out_path)
{
...
// PCM WAV helpers
static bool pcm_wav_write_header(FILE *f, int channels, int sample_rate, int bits, uint64_t data_bytes)
{
    if (!f)
        return false;
...
static bool pcm_wav_start(const char *path, int channels, int sample_rate, int bits)
{
    if (!path)
        return false;
...
static void pcm_wav_write_samples(int16_t *left, int16_t *right, int frames)
{
...
static bool midi_record_stop(void)
{
    if (!g_midi_recording)
        return false;
...
static bool bae_load_song(const char *path)
{
    ...
    if (le)
    {
        if (strcmp(ext, ".wav") == 0 || strcmp(ext, ".aif") == 0 || strcmp(ext, ".aiff") == 0 || strcmp(ext, ".au") == 0 || strcmp(ext, ".mp2") == 0 || strcmp(ext, ".mp3") == 0)
        {
            isAudio = true;
        }
    }
...
static void bae_set_volume(int volPct)
{
    ...
    {
        BAEMixer_SetMasterVolume(g_bae.mixer, FLOAT_TO_UNSIGNED_FIXED(f));
    }
}
...
static void bae_set_reverb(int idx)
{
    if (g_bae.mixer)
    {
        if (idx < 0)
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        if (!g_font)
        {
            ...
            for (int i = 0; tryFonts[i]; ++i)
            {
                if (!g_font)
                {
                    g_font = TTF_OpenFont(tryFonts[i], 14);
                }
            }
            if (g_font)
            {
                BAE_PRINTF("Loaded system TTF font.\n");
            }
        }
    }
    ...
    {
        BAE_PRINTF("miniBAE init failed\n");
    }
    if (!bae_init(g_sample_rate_hz, g_stereo_output))
    {
        BAE_PRINTF("miniBAE init failed (retry)\n");
    }
                ...
                if (dropped)
                {
                    ...
                    if (is_bank_file)
                    {
                        ...
                        if (load_bank(dropped, playing, transpose, tempo, volume, loopPlay, reverbType, ch_enable, true))
                        {
                            ...
                            {
                                char msg[160];
                                snprintf(msg, sizeof(msg), "Loaded bank: %s", fn);
                                set_status_message(msg);
                            }
                        }
                        else
...
#undef FORWARD_OUT
        ...
        if (showKeyboard)
        {
            ...
            for (int n = firstNote; n <= lastNote; n++)
            {
                ...
                if (isWhite)
                {
                    ...
                    if (g_keyboard_mouse_note == n)
                        keyCol = g_highlight_color; // mouse-held note priority
                    ...
                    if (!g_keyboard_channel_dd_open && !modal_block && !g_reverbDropdownOpen && !g_exportDropdownOpen && !g_exporting && ui_mx >= x && ui_mx < x + w && ui_my >= kbY && ui_my < kbY + kbH)
                    {
            ...
            for (int n = firstNote; n <= lastNote; n++)
            {
                ...
                if (isBlack)
                {
                    ...
                    if (g_keyboard_mouse_note == n)
                        keyCol = g_accent_color; // invert colors for contrast
                    draw_rect(R, (Rect){bx, kbY, bw, bh}, keyCol);
                    draw_frame(R, (Rect){bx, kbY, bw, bh}, g_panel_border);
                    if (!g_keyboard_channel_dd_open && !modal_block && !g_reverbDropdownOpen && !g_exportDropdownOpen && !g_exporting && ui_mx >= bx && ui_mx < bx + bw && ui_my >= kbY && ui_my < kbY + bh)
                    {
        ...
        }
        {
            ...
            else
            {
                if (!modal_block)
                {
                    if (ui_toggle(R, loopR, &loopPlay, "Loop", ui_mx, ui_my, ui_mclick))
                        clicked = true;
                }
                else if (g_exporting)
                {
                    // While exporting allow loop toggle using real mouse coords so user can uncheck loop
                    if (ui_toggle(R, loopR, &loopPlay, "Loop", mx, my, mclick))
                        clicked = true;
                }
                if (clicked)
                {
        ...
        if (g_midi_input_enabled || (!g_bae.is_audio_file && g_bae.song_loaded))
        {
            ...
            if (g_midi_input_enabled)
            {
                ...
                SDL_Color disabledBg = g_panel_bg;
                SDL_Color disabledTxt = g_panel_border;
                Rect placeholder = {320, 215, 80, 22};
                draw_rect(R, placeholder, disabledBg);
                draw_frame(R, placeholder, g_panel_border);
                draw_text(R, placeholder.x + 14, placeholder.y + 4, "Record", disabledTxt);
            }
            else
            {
                        ...
                        if (export_file)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (result != BAE_NO_ERROR)
                                    {
                                        BAE_PRINTF("Export: BAESong_Start failed (%d)\n", result);
                                    }
                                    else
                                    {
                                        g_bae.is_playing = true;
        ...
        if (g_midi_input_enabled)
        {
            ...
            Rect recRect = {320, 215, 80, 22};
            // Dropdown button to right of record (two-column list)
            Rect recFmtBtn = {recRect.x + recRect.w + 8, 215, 200, 22};
                        ...
                        else
                        {
                            ...
                            if (export_file)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (selectedCodecIndex >= (int)(sizeof(g_exportCompressionMap) / sizeof(g_exportCompressionMap[0])))
                                        selectedCodecIndex = (int)(sizeof(g_exportCompressionMap) / sizeof(g_exportCompressionMap[0])) - 1;
                                    compression = g_exportCompressionMap[selectedCodecIndex];
                                }
#else
                                if (g_midiRecordFormatIndex != 1)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    char msg[128];
                                                    snprintf(msg, sizeof(msg), "Export BAESong_Start failed (%d)", result);
                                                    set_status_message(msg);
                                                    BAEMixer_StopOutputToFile();
                                                    free(export_file);
                                                }
                                                else
                                                {
                                                    ...
                                                    strncpy(g_export_path, export_file, sizeof(g_export_path) - 1);
                                                    g_export_path[sizeof(g_export_path) - 1] = '\0';
                                                    g_exportCodecIndex = selectedCodecIndex;
                                                    g_export_realtime_mode = true;
                                                    set_status_message("Export started");
                                                    free(export_file);
                                                }
...
#else
                                        // no MPEG support and not WAV => fail
                                        set_status_message("MP3 export not supported in this build");
                                        free(export_file);
#endif
                                    }
            ...
            else
            {
                ...
                {
                    // If MIDI recording is active, stop and save
                    if (g_midiRecordFormatIndex == 0)
                    {
                        if (!midi_record_stop())
                        {
                    ...
                    else if (g_midiRecordFormatIndex == 1)
                    {
...
// Builtin Bank button (left of Load Bank)
            ...
            int btw = 0, bth = 0;
            measure_text("Builtin Bank", &btw, &bth);
        ...
        if (g_show_settings_dialog)
        {
            ...
            draw_text(R, expRect.x + 6, expRect.y + 6, expName, exp_txt);
            ...
            draw_text(R, midiDevRect.x + 6, midiDevRect.y + 6, curDev, md_txt);
            draw_text(R, midiDevRect.x + midiDevRect.w - 16, midiDevRect.y + 6, g_midi_input_device_dd_open ? "^" : "v", md_txt);
            ...
            draw_text(R, midiOutDevRect.x + 6, midiOutDevRect.y + 6, curOutDev, mo_txt);
            draw_text(R, midiOutDevRect.x + midiOutDevRect.w - 16, midiOutDevRect.y + 6, g_midi_output_device_dd_open ? "^" : "v", mo_txt);
        ...
        if (g_show_about_dialog)
        {
            ...
            draw_text(R, navPrev.x + 6, navPrev.y + 3, "<", g_button_text);
            draw_rect(R, navNext, overNext ? g_button_hover : g_button_base);
            draw_frame(R, navNext, g_button_border);
            draw_text(R, navNext.x + 6, navNext.y + 3, ">", g_button_text);
            // Page indicator
            char pg[32];
