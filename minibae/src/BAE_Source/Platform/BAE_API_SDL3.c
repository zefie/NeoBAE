...
// SDL3 platform implementation of subset of BAE_API.
...
// Threading for audio pumping - SDL3 requires us to feed data to streams
static SDL_Thread *g_audioThread = NULL;
...
// Audio thread function that feeds data to SDL3 audio stream
static int audio_thread_func(void *data)
{
    (void)data;
    BAE_PRINTF("SDL3 audio thread started\n");
    
    // Target timing: feed data to maintain ~2-3 buffer's worth in the stream
    const uint32_t targetDelayMs = 5; // 5ms between feeds
    
    while (g_audioThreadRunning)
    {
        if (g_muted)
        {
            SDL_Delay(targetDelayMs);
            continue;
        }
        ...
        // Check how much data is queued in the stream
        size_t queued = SDL_GetAudioStreamQueued(g_audioStream);
        size_t available = SDL_GetAudioStreamAvailable(g_audioStream);
        
        // Only generate more data if we're running low
        if (queued < (size_t)(g_audioByteBufferSize * 2)) // Keep 2 slices queued
        {
            ...
            BAE_BuildMixerSlice(g_threadContext, g_sliceStatic, sliceBytes, frames);

            // Handle recording callbacks - same logic as SDL2 version
            if (g_pcm_rec_fp)
            {
            ...
            {
                const uint32_t frames_u32 = (uint32_t)(sliceBytes / (g_channels * (g_bits / 8)));
                if (frames_u32 && g_bits == 16)
                {
                    int16_t *samples = (int16_t *)g_sliceStatic;
                    if (g_channels == 1)
                    {
                        g_flac_recorder_callback(samples, samples, frames_u32);
                    }
                    else if (g_channels == 2)
                    {
                        ...
                        if (frames_u32 > temp_frames)
                        {
                            int16_t *new_left = (int16_t *)malloc(frames_u32 * sizeof(int16_t));
                            int16_t *new_right = (int16_t *)malloc(frames_u32 * sizeof(int16_t));
                        ...
                        {
                            for (uint32_t i = 0; i < frames_u32; i++)
                            {
                                left_temp[i] = samples[i * 2];
                                right_temp[i] = samples[i * 2 + 1];
                            }
                            g_flac_recorder_callback(left_temp, right_temp, frames_u32);
                        }
                    }
                }
            }
            ...
            if (g_vorbis_recorder_callback)
            {
                ...
                if (frames_u32 && g_bits == 16)
                {
                    ...
                    if (g_channels == 1)
                    {
                        g_vorbis_recorder_callback(samples, samples, frames_u32);
                    }
                    else if (g_channels == 2)
                    {
                        static int16_t *left_temp = NULL, *right_temp = NULL;
                        ...
                        if (left_temp && right_temp)
                        {
                            ...
                            g_vorbis_recorder_callback(left_temp, right_temp, frames_u32);
                        }
                    }
                }
            }
            ...
            // MP3 recorder - same logic as SDL2 version
            if (g_mp3enc && g_mp3enc->accepting)
            {
                ...
                if (frames_u32)
                {
                    ...
                    static uint32_t scratchFrames = 0;
                    int16_t *temp = NULL;
                    if (g_bits == 16)
                    {
                        temp = (int16_t *)g_sliceStatic;
                    }
                    else
                    {
                        if (scratchFrames < frames_u32)
                        {
                            free(scratch16);
                            scratch16 = (int16_t *)malloc((size_t)frames_u32 * s->channels * sizeof(int16_t));
                            scratchFrames = scratch16 ? frames_u32 : 0;
                        }
                        if (!scratch16)
                        {
                            s->droppedFrames += frames_u32;
                            goto after_ring_write;
                        }
                        const uint8_t *src8 = (const uint8_t *)g_sliceStatic;
                        for (uint32_t i = 0; i < frames_u32 * s->channels; i++)
                    ...
                    uint32_t toWrite = (frames_u32 <= space) ? frames_u32 : space;
                    if (toWrite > 0)
                    {
        ...
        after_ring_write:
            ...
            // Feed the data to SDL3 audio stream
            if (SDL_PutAudioStreamData(g_audioStream, g_sliceStatic, sliceBytes) < 0)
            {
                BAE_PRINTF("SDL_PutAudioStreamData failed: %s\n", SDL_GetError());
            }
            else
            {
    ...
    BAE_PRINTF("SDL3 audio thread stopped\n");
    return 0;
}
...
int BAE_AcquireAudioCard(void *threadContext, uint32_t sampleRate, uint32_t channels, uint32_t bits)
{
    ...
    BAE_PRINTF("Computing provisional slice size from engine (pre SDL3 stream creation)...\n");
    ...
    if (!g_audioMutex)
    {
        BAE_PRINTF("Failed to create audio mutex: %s\n", SDL_GetError());
        return -1;
    }

    // Prepare SDL_AudioSpec
    SDL_AudioSpec spec;
    SDL_zero(spec);
    ...
    BAE_PRINTF("Opening SDL3 audio device stream: freq=%d, channels=%d, format=0x%x\n",
               spec.freq, spec.channels, spec.format);

    // SDL3: Use simplified stream creation that opens device and creates stream
    g_audioStream = SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, 
                                              &spec, 
                                              NULL,  // No callback - we'll push data manually
                                              NULL); // No userdata
    if (!g_audioStream)
    {
    ...
    g_audioDevice = SDL_GetAudioStreamDevice(g_audioStream);
    BAE_PRINTF("SDL3 audio stream created successfully, device ID: %u\n", g_audioDevice);

    // Get the actual format that was set
    SDL_AudioSpec actual_spec;
    if (SDL_GetAudioStreamFormat(g_audioStream, &actual_spec, NULL) == 0)
    {
        BAE_PRINTF("SDL3 actual format: %d Hz, %d ch, fmt 0x%x\n",
                   actual_spec.freq, actual_spec.channels, actual_spec.format);

        // Update our parameters if SDL3 changed them
        if ((uint32_t)actual_spec.freq != g_sampleRate || (uint32_t)actual_spec.channels != g_channels)
        {
            BAE_PRINTF("SDL3 adjusted audio format (requested %u Hz/%u ch -> got %d Hz/%d ch). Recomputing slice.\n",
                       g_sampleRate, g_channels, actual_spec.freq, actual_spec.channels);
            g_sampleRate = (uint32_t)actual_spec.freq;
            g_channels = (uint32_t)actual_spec.channels;
            PV_ComputeSliceSizeFromEngine();
        }
    }
    ...
    g_audioThread = SDL_CreateThread(audio_thread_func, "miniBAE_SDL3_Audio", NULL);
    if (!g_audioThread)
    {
        BAE_PRINTF("Failed to create audio thread: %s\n", SDL_GetError());
    ...
    // Resume the audio device (SDL3 streams start paused)
    if (SDL_ResumeAudioStreamDevice(g_audioStream) < 0)
    {
        BAE_PRINTF("Failed to resume audio stream device: %s\n", SDL_GetError());
        g_audioThreadRunning = 0;
        SDL_WaitThread(g_audioThread, NULL);
        g_audioThread = NULL;
        SDL_DestroyAudioStream(g_audioStream);
        g_audioStream = NULL;
        g_audioDevice = 0;
    ...
    BAE_PRINTF("SDL3 audio system active: %u Hz, %u ch, slice %u frames (%d bytes)\n",
               g_sampleRate, g_channels, g_framesPerSlice, g_audioByteBufferSize);
    return 0;
}
...
int BAE_ReleaseAudioCard(void *threadContext)
{
    ...
    BAE_PRINTF("Releasing SDL3 audio resources...\n");
    ...
    if (g_audioThread)
    {
        ...
        SDL_WaitThread(g_audioThread, NULL);
        g_audioThread = NULL;
        BAE_PRINTF("Audio thread stopped\n");
    }
    
    // Destroy stream (this also closes the device in SDL3)
    if (g_audioStream)
    {
        SDL_DestroyAudioStream(g_audioStream);
        g_audioStream = NULL;
        g_audioDevice = 0;
        BAE_PRINTF("Audio stream destroyed\n");
    }
...
void BAE_GetDeviceName(int32_t deviceID, char *cName, uint32_t cNameLength)
{
    ...
    {
        snprintf(cName, cNameLength, "SDL3,stream,threaded");
    }
}
...
static XBOOL MP3Refill_FromRing(void *buffer, void *userRef)
{
    ...
    while (copied < needFrames)
    {
        while (s->usedFrames == 0)
        {
            ...
            SDL_WaitCondition(s->cond, s->mtx);
        }
...
void BAE_Platform_MP3Recorder_Stop(void)
{
    ...
    if (!s)
        return;
    // Lock audio thread to prevent callback from writing concurrently while we flip flags
    if (g_audioMutex)
        SDL_LockMutex(g_audioMutex);
